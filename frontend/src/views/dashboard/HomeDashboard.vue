<template>
  <div class="home-dashboard">
    ...
    <!-- 数据加载中 -->
    <div v-if="dashboardLoading" style="text-align:center;padding:40px;color:var(--n-600);">
    ...
    <el-row v-show="!dashboardLoading" :gutter="16" class="kpi-row">
      <el-col :span="6" v-for="kpi in kpis" :key="kpi.label">
        <div class="kpi-card" @click="goPage(kpi.link)" :title="kpi.label + ' — 点击查看详情'">
          <div class="kpi-top">
            <span class="kpi-icon" :style="{ background: kpi.color }"><el-icon :size="18" color="var(--n-0)"><component :is="iconMap[kpi.icon]" /></el-icon></span>
            <span class="kpi-trend" :class="kpi.trend > 0 ? 'up' : kpi.trend < 0 ? 'down' : 'flat'" :style="{ color: kpi.trendColor, background: kpi.trend > 0 ? 'var(--ok-100)' : kpi.trend < 0 ? 'var(--bad-100)' : 'var(--n-50)' }" v-if="kpi.trend !== 0">
    ...
    <div class="section-card" v-show="!dashboardLoading" style="margin-top:16px">
      <div class="section-header">
        ...
        <div style="display:flex;align-items:center;gap:8px">
          ...
          <span style="font-size:12px;color:var(--n-600)" v-else><el-icon :size="13" color="var(--ok-600)" style="vertical-align:-2px;margin-right:4px"><CircleCheck /></el-icon>今日无待办，一切正常</span>
        </div>
      </div>
        ...
        <el-row :gutter="12">
          <el-col :span="8" v-for="todo in todoList" :key="todo.key" style="margin-bottom:12px">
            <div class="todo-item" @click="goPage(todo.link)" :title="todo.title + ' — 点击处理'">
              <span class="todo-icon" :style="{ background: todoTypeColor[todo.type] || 'var(--n-600)' }"><el-icon :size="18" color="var(--n-0)"><component :is="iconMap[todoSeverityMap[todo.severity]?.icon || 'list']" /></el-icon></span>
              <div class="todo-body">
      ...
      <div v-else class="empty-state">
        <el-icon :size="30" color="var(--brand-100)"><CircleCheck /></el-icon>
    ...
    <el-row :gutter="16" style="margin-top:16px">
      ...
      <el-col :span="10">
        <div class="section-card" style="margin-bottom:16px">
          ...
          <div v-else style="text-align:center;color:var(--n-300);font-size:13px;padding:16px 0">暂无逾期账单</div>
        </div>
        ...
        <div class="section-card" style="margin-bottom:16px">
          ...
          <div v-else style="text-align:center;color:var(--n-300);font-size:13px;padding:16px 0">暂无催缴数据</div>
        </div>
        ...
        <div class="section-card">
          ...
          <div v-else style="text-align:center;color:var(--n-300);font-size:13px;padding:16px 0">暂无即将到期合同</div>
        </div>
      </el-col>
    </el-row>
    ...
    <div class="section-card" style="margin-top:16px" v-show="!dashboardLoading">
        ...
        <el-row :gutter="16">
          <el-col :span="10">
            <div style="text-align:center">
              <div style="font-size:34px;font-weight:700;color:var(--n-900)">{{ roomStats.occupancyRate }}%</div>
              <div style="font-size:12px;color:var(--n-600);margin-top:2px">总房源 {{ roomStats.total }} 间 · 入住率</div>
            </div>
            <div class="room-legend" style="margin-top:12px">
              <div class="room-legend-item" v-for="s in roomStatusList.filter(x => x.count > 0)" :key="s.name">
                <span class="room-dot" :style="{ background: roomStatusColorMap[s.name] || 'var(--n-600)' }"></span>
          ...
          <el-col :span="10">
            ...
            <div class="structure-list" style="margin-top:14px">
              <div class="structure-item" v-for="s in (propOps.structure?.workByType || [])" :key="s.name">
                ...
                <div class="viz-bar-wrap"><span class="viz-bar-fill" :style="{ width: (s.value / (Math.max(1, ...(propOps.structure?.workByType || []).map((x: any) => x.value))) * 100) + '%', background: 'var(--brand-600)' }"></span></div>
      ...
      <div v-if="opsTab === 'fire'">
        <el-row :gutter="16">
          <el-col :span="10">
            <div class="ops-kpi-row">
              <div class="ops-kpi"><div class="ops-kpi-val" style="color:var(--brand-600)">{{ fireDash.totalEquipment }}</div><div class="ops-kpi-label">器材总数</div></div>
              <div class="ops-kpi"><div class="ops-kpi-val" style="color:var(--warn-600)">{{ fireDash.expiringEquipment }}</div><div class="ops-kpi-label">过期/到期</div></div>
              <div class="ops-kpi"><div class="ops-kpi-val" style="color:var(--bad-600)">{{ fireDash.pendingViolations }}</div><div class="ops-kpi-label">隐患待整改</div></div>
              <div class="ops-kpi"><div class="ops-kpi-val" style="color:var(--brand-600)">{{ fireDash.monthInspections }}</div><div class="ops-kpi-label">本月检查</div></div>
            </div>
...
const kpis = ref<any[]>([
  { label: '房源总数', value: '--', color: tokens.brand600, icon: 'home', link: '/rent/properties', trend: 0, percent: 0, barColor: tokens.brand600, trendColor: tokens.n600 },
  { label: '在租合同', value: '--', color: tokens.brand300, icon: 'users', link: '/contract/list', trend: 0, percent: 0, barColor: tokens.brand300, trendColor: tokens.n600 },
  { label: '当月应收(万)', value: '--', color: tokens.brand300, icon: 'money', link: '/rent/bills', trend: 0, percent: 0, barColor: tokens.brand300, trendColor: tokens.n600 },
  { label: '收缴率', value: '--', color: tokens.brand600, icon: 'trend', link: '/rent/dashboard', trend: 0, percent: 0, barColor: tokens.brand600, trendColor: tokens.n600 },
]);

const dunningStats = ref([
  { level: '到期提醒', count: 0, color: tokens.brand600 },
  { level: '一级催缴', count: 0, color: tokens.warn600 },
  { level: '二级催缴', count: 0, color: tokens.warn600 },
  { level: '三级催缴', count: 0, color: tokens.bad600 },
]);
...
const todoTypeColor: Record<string, string> = { '财务': tokens.bad600, '合同': tokens.brand600, '物业': tokens.brand600, '消防': tokens.warn600 };
...
const roomChartOption = computed(() => ({
  ...
  series: [{ type: 'pie', radius: ['40%', '68%'], center: ['50%', '44%'], data: roomStatusList.value.filter(s => s.count > 0).map(s => ({ name: s.name, value: s.count, itemStyle: { color: roomStatusColorMap[s.name] || tokens.n600 } })), label: { fontSize: 11 }, emphasis: { itemStyle: { shadowBlur: 10, shadowOffsetX: 0, shadowColor: tokens.scrim } } }],
}));
const propTrendChartOption = computed(() => {
  ...
  return {
    ...
    series: [
      { name: '实收', type: 'line', smooth: true, data: (t.collectedTrend || []).map((x: any) => x.paid), lineStyle: { color: tokens.brand600, width: 2 }, itemStyle: { color: tokens.brand600 }, symbol: 'circle', symbolSize: 5 },
      { name: '应收', type: 'line', smooth: true, data: (t.collectedTrend || []).map((x: any) => x.total), lineStyle: { color: tokens.brand300, width: 1.5 }, itemStyle: { color: tokens.brand300 }, symbol: 'circle', symbolSize: 5 },
    ],
  };
});
...
const roomStatusColorMap: Record<string, string> = { '空置': tokens.n600, '已锁定': tokens.brand300, '已预订': tokens.stBooked, '已出租': tokens.brand600, '退租中': tokens.warn600, '待保洁': tokens.info100, '待验收': tokens.warn600, '维修中': tokens.warn600, '已冻结': tokens.n400 };
...
const chartOption = computed(() => {
  ...
  const series: any[] = [
    { name: '应收金额', type: 'bar', data: data.map((t: any) => +(t.due / 10000).toFixed(1)), itemStyle: { color: tokens.brand300, borderRadius: [3,3,0,0] }, barMaxWidth: 24 },
    { name: '实收金额', type: 'line', smooth: true, data: data.map((t: any) => +(t.collected / 10000).toFixed(1)), lineStyle: { color: tokens.brand600, width: 2 }, itemStyle: { color: tokens.brand600 }, symbol: 'circle', symbolSize: 6 },
    { name: '逾期率', type: 'line', smooth: true, yAxisIndex: 1, data: data.map((t: any) => t.rate || 0), lineStyle: { color: tokens.bad600, width: 1.5, type: 'dashed' }, itemStyle: { color: tokens.bad600 }, symbol: 'diamond', symbolSize: 6 },
  ];
  if (showCumulative.value) {
    series.push(
      { name: '累计应收', type: 'line', smooth: true, data: dueCum, lineStyle: { color: tokens.brand600, width: 1.5, type: 'dotted' }, itemStyle: { color: tokens.brand600 }, symbol: 'circle', symbolSize: 4 },
      { name: '累计实收', type: 'line', smooth: true, data: colCum, lineStyle: { color: tokens.ok600, width: 1.5, type: 'dotted' }, itemStyle: { color: tokens.ok600 }, symbol: 'circle', symbolSize: 4 },
    );
  }
  return {
    tooltip: {
...
async function loadDashboardData(silent = false) {
  ...
  try {
    ...
    const barColor = (v: number) => v >= 90 ? tokens.brand600 : v >= 70 ? tokens.brand300 : v >= 50 ? tokens.brand300 : tokens.brand300;
    const trendColor = (t: number) => t > 0 ? tokens.brand600 : t < 0 ? tokens.bad600 : tokens.n600;

    kpis.value = [
      { label: '房源总数', value: String(o.totalProperties || 0), color: tokens.brand600, icon: 'home', link: '/rent/properties', trend: 0, percent: occRate, barColor: barColor(occRate), trendColor: trendColor(0) },
      { label: '在租合同', value: String(o.activeContracts || 0), color: tokens.brand300, icon: 'users', link: '/contract/list', trend: 0, percent: rentRate, barColor: barColor(rentRate), trendColor: trendColor(0) },
      { label: '当月应收(万)', value: ((d.monthlyDue || 0) / 10000).toFixed(1), color: tokens.brand300, icon: 'money', link: '/rent/bills', trend: dueTrend, percent: crVal, barColor: barColor(crVal), trendColor: trendColor(dueTrend) },
      { label: '收缴率', value: crVal + '%', color: tokens.brand600, icon: 'trend', link: '/rent/dashboard', trend: collectedTrend, percent: crVal, barColor: barColor(crVal), trendColor: trendColor(collectedTrend) },
    ];

    if (d.dunningStats) dunningStats.value = d.dunningStats.map((s: any, i: number) => ({
      ...s, count: s.count || 0, color: dunningStats.value[i]?.color || tokens.n600,
    }));
...
.section-card { background: $n-0; border: 1px solid $n-200; border-radius: $r-box; padding: 20px; }
.section-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 14px; }
.section-title { font-size: 14px; font-weight: 600; color: $n-900; }

// ---- KPI ----
.kpi-card {
  background: $n-0; border: 1px solid $n-200; border-radius: $r-box; padding: 20px 16px; text-align: center;
  cursor: pointer;
  transition: transform 0.25s, box-shadow 0.25s;
  &:hover { transform: translateY(-2px); box-shadow: $sh-1; }
  &:active { transform: scale(0.98); }
}
.kpi-top { display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px; }
.kpi-icon { width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 18px; color: $n-0; }
.kpi-trend { font-size: 11px; font-weight: 600; padding: 1px 6px; border-radius: 10px; }
.kpi-trend.up { color: $brand-600; background: $brand-100; }
.kpi-trend.down { color: $bad-600; background: $bad-100; }
.kpi-trend.flat { color: $n-600; background: $n-50; }
.kpi-value { font-size: 28px; font-weight: 700; margin: 4px 0; }
.kpi-label { font-size: 12px; color: $n-600; margin-bottom: 10px; }
.kpi-bar { height: 4px; background: $n-100; border-radius: 2px; overflow: hidden; }
...
.quick-btn {
  ...
  background: $n-0; border: 1px solid $n-200; margin-bottom: 12px; user-select: none; font-size: 13px; color: $n-700;
  &:hover { background: $n-50; transform: translateY(-2px); box-shadow: $sh-1; }
...
.viz-label { font-size: 12px; color: $n-700; width: 70px; flex-shrink: 0; }
.viz-bar-wrap { flex: 1; height: 10px; background: $n-100; border-radius: 5px; overflow: hidden; margin: 0 10px; }
...
.expiry-item { display: flex; align-items: center; justify-content: space-between; padding: 10px 12px; border-radius: 12px; cursor: pointer; transition: background 0.2s; background: $n-0;
  &:hover { background: $n-50; } }
.expiry-no { font-size: 13px; color: $n-900; font-weight: 500; }
.expiry-date { font-size: 12px; color: $n-600; }
.overdue-amt { font-size: 12px; color: $bad-600; font-weight: 600; }

// ---- 系统信息 ----
.sys-info-card { margin-top: 16px; }
.sys-info-row { display: flex; align-items: center; flex-wrap: wrap; font-size: 12px; color: $n-600; }
.sys-dot { width: 6px; height: 6px; border-radius: 50%; background: $brand-600; display: inline-block; margin-right: 4px; }
.sys-sep { margin: 0 14px; color: $n-200; }
...
.bar-stack { flex: 1; width: 100%; max-width: 40px; display: flex; flex-direction: column; justify-content: flex-end; gap: 2px; border-radius: 4px 4px 0 0; overflow: hidden; background: $n-50; }
.bar-due { background: $brand-300; border-radius: 2px 2px 0 0; transition: height 0.6s ease; min-height: 2px; }
.bar-collected { background: $brand-600; border-radius: 2px 2px 0 0; transition: height 0.6s ease; min-height: 2px; }
.bar-label { font-size: 10px; color: $n-600; margin-top: 4px; }
.trend-legend { position: absolute; top: 0; right: 8px; display: flex; gap: 12px; font-size: 11px; color: $n-700; }
.legend-dot { display: inline-block; width: 8px; height: 8px; border-radius: 2px; margin-right: 4px; }
.due-dot { background: $brand-300; }
.collected-dot { background: $brand-600; }
.chart-controls { display: flex; align-items: center; }

// ---- 今日待办 ----
.todo-item { display: flex; align-items: center; gap: 10px; padding: 12px 14px; border-radius: $r-box; background: $n-0; border: 1px solid $n-200; cursor: pointer; transition: all 0.2s;
  &:hover { background: $n-50; transform: translateY(-2px); box-shadow: $sh-1; } }
.todo-icon { width: 34px; height: 34px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 18px; color: $n-0; flex-shrink: 0; }
.todo-body { flex: 1; min-width: 0; }
.todo-title { font-size: 13px; font-weight: 600; color: $n-900; display: flex; align-items: center; gap: 6px; }
.todo-sub { font-size: 12px; color: $n-600; margin-top: 2px; }
...
.room-name { font-size: 12px; color: $n-700; width: 46px; flex-shrink: 0; }
.room-bar-wrap { flex: 1; height: 8px; background: $n-100; border-radius: 4px; overflow: hidden; }
...
.ops-kpi { background: $n-0; border: 1px solid $n-200; border-radius: $r-box; padding: 12px 14px; text-align: center; }
.ops-kpi-val { font-size: 22px; font-weight: 700; color: $n-900; }
.ops-kpi-label { font-size: 12px; color: $n-600; margin-top: 4px; }
.structure-list { display: flex; flex-direction: column; gap: 10px; }
.structure-item { display: flex; align-items: center; gap: 8px; }
.structure-name { font-size: 12px; color: $n-700; width: 70px; flex-shrink: 0; }
...
.empty-sub { font-size: 12px; color: $n-600; }
</style>
