...
2. 双击运行 **`setup.bat`**，等待自动完成所有依赖安装

3. （可选）修改 `.env` 文件中的数据库密码（默认 root 空密码）

4. **完成！** 系统已就绪，首次启动时会自动创建数据库和默认管理员账号

> **注意：** 系统启动时会自动执行 `CREATE DATABASE IF NOT EXISTS` 和默认管理员创建，无需手动操作数据库。
   DB_PORT=3306
...
**双击 `start-dev.bat`**，系统将自动：

1. 检查 MySQL 和 Redis 连接状态
2. 自动创建数据库（如不存在）
3. 自动创建默认管理员账号（如不存在）
4. 同步数据库表结构
5. 启动后端 API 服务 (端口 3001)
6. 启动前端开发服务器 (端口 5173)
...
**双击 `start.bat`**，系统将先编译前端静态资源，再启动服务。
